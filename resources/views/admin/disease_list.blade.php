                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="dataTables_filter">
                            ...
                            <label><input name="search" value="<?= $search_txt ?? '' ?>" placeholder="Type and Hit Tab"
                                    type="text" class="input-xs form-control search " style="    height: 35px;"></label>
                        </div>
                        <div id="table_data" class="result_append">


                            @include('admin.disease_row', ['disease' => $disease])
                        </div>
        ...
        <div class="modal-dialog modal-xl">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title" id="myLargeModalLabel">Disease Detail</h4>
    ...
    <script>
        $(document).ready(function() {
                ...
                if (result) {
                    $.get(link, function(data) {
                        if (data.status == 'success') {
                            $.toast({
                                heading: 'Message',
                                text: 'Disease Deleted Successfully',
                                position: 'top-right',
                                loaderBg: '#ff6849',
                                icon: 'success',
                                hideAfter: 3500,
                            ...
                            $.toast({
                                heading: 'Message',
                                text: 'Disease Not Deleted Because Student Registered',
                                position: 'top-right',
                                loaderBg: '#ff6849',
                                icon: 'warning',
                                hideAfter: 3500,
            ...
            $(document).on('click', 'input[name="user_status"]', function() {
                ...
                $.get('{{ url('disease/active-inactive') }}' + '/' + attrfr, function(data) {
            ...
            $(document).on('click', '.openDetailModal', function() {
                start_loader()
                var attrfr = $(this).attr('eid');
                $.get('{{ url('disease/show') }}' + '/' + attrfr, function(data) {
            ...
            $(document).on('blur', '.search', function() {
                ...
                start_loader();
                $.get('{{ url('disease/search') }}', searchParams, function(data) {
            ...
            function fetch_data(page) {
                start_loader();
                $.ajax({
                    url: "{{ url('/disease/search?page=') }}" + page,
                    type: 'GET',
                    data: searchParams,
